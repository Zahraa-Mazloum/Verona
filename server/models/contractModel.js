...
const contractSchema = new mongoose.Schema({
  ...
  contractTime: {
    type: String,
    enum: ['month', 'Three months', 'Six months', 'year', 'day'], 
    required: true 
  },
  contractTime_ar: { 
    type: String, 
    enum: ['شهر', 'سنة', 'يوم' ,'سته اشهر' , 'ثلاثة اشهر'],
    required: true 
  },
  startDate: { 
    type: Date, 
...
contractSchema.pre('save', function(next) {
  if (this.isModified('startDate') || this.isNew) {
    ...
    } else if (this.contractTime === 'Three months' || this.contractTime === 'ثلاثة اشهر') {
      endDate.setMonth(startDate.getMonth() + 3);
      this.contractPercentage = 6.9; 
    } else if (this.contractTime === 'Six months' || this.contractTime === 'سته اشهر') {
      endDate.setMonth(startDate.getMonth() + 6);
      this.contractPercentage = 15.6; 
    } else if (this.contractTime === 'day' || this.contractTime === 'يوم') {
      endDate.setDate(startDate.getDate() + 1);
      this.contractPercentage = 1; 
    }
...
contractSchema.virtual('currentProfit').get(function() {
  ...
  const duration = {
    ...
    'Three months': 3 * 30 * 24 * 60 * 60 * 1000, 
    'Six months': 6 * 30 * 24 * 60 * 60 * 1000, 
