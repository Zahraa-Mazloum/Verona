...
const InvestmentTypesTable = () => {
  ...
  const filteredtypes = types.filter(investmentTypes =>
    safeLowerCase(i18n.language === 'en' ? investmentTypes.title_en : investmentTypes.title_ar).includes(safeLowerCase(search)) ||
    safeLowerCase(i18n.language === 'en' ? investmentTypes.description : investmentTypes.description_ar).includes(safeLowerCase(search))
  );

  const columns = [
    {
      field: i18n.language === 'ar' ? 'title_ar' : 'title_en',
      headerName: t('name'),
      flex: 1
    },
    {
...
export default InvestmentTypesTable;
